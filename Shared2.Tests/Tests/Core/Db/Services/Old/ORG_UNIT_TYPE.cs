...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class ORG_UNIT_TYPE_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        private static ORG_UNIT_TYPE_Repository Setup()
        {
            return new ORG_UNIT_TYPE_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new ORG_UNIT_TYPE
            {
        ...
        {
            /*var entity_to_create = new ORG_UNIT_TYPE
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new ORG_UNIT_TYPE
            {
                ID = 0, // не обновляем
        ...
        private static ORG_UNIT_TYPE Upd(ORG_UNIT_TYPE u)
        {
            /*var model = new ORG_UNIT_TYPE
            {
                ID = u.ID,
        ...
        private static ORG_UNIT_TYPE Create(ORG_UNIT_TYPE model)
        {
            // предпроверка
        ...
        private static ORG_UNIT_TYPE Read(int? id)
        {
            // подготовка
            var repository = Setup();

            ORG_UNIT_TYPE e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(ORG_UNIT_TYPE model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(ORG_UNIT_TYPE model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
