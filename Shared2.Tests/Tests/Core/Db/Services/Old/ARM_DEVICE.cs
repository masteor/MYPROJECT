...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class ARM_DEVICE_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        public static ARM_DEVICE_Repository Setup()
        {
            return new ARM_DEVICE_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new ARM_DEVICE
            {
        ...
        {
            /*var entity_to_create = new ARM_DEVICE
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new ARM_DEVICE
            {
                ID = 0, // не обновляем
            ...
            e.ID_ARM = entity_to_update.ID_ARM;
            e.ID_DEVICE = entity_to_update.ID_DEVICE;
            e.ID_REQUEST_1 = entity_to_update.ID_REQUEST_1;
            e.ID_REQUEST_2 = entity_to_update.ID_REQUEST_2;
            e.ID_DEVICE_PERM = entity_to_update.ID_DEVICE_PERM;
            ...
            Read(e.ID).Should().BeEquivalentTo(entity_to_update, opt => opt
                .Excluding(o=>o.ID)
                .Excluding(o=>o.ARM)
                .Excluding(o=>o.DEVICE)
                .Excluding(o=>o.REQUEST)
                .Excluding(o=>o.REQUEST1)
                .Excluding(o=>o.DEVICE_PERM)); // ID не проверяется, все остальные проверяются
        ...
        private static ARM_DEVICE Upd(ARM_DEVICE u)
        {
            /*var model = new ARM_DEVICE
            {
                ID = u.ID,
            ...
            var model = new ARM_DEVICE
            {
                ID = u.ID,
                ID_ARM = u.ID_ARM,
                ID_DEVICE = u.ID_DEVICE,
                ID_DEVICE_PERM = u.ID_DEVICE_PERM,
        ...
        private static ARM_DEVICE Create(ARM_DEVICE model)
        {
            // предпроверка
        ...
        private static ARM_DEVICE Read(int? id)
        {
            // подготовка
            var repository = Setup();
        ...
        private void Delete(ARM_DEVICE model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(ARM_DEVICE model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
