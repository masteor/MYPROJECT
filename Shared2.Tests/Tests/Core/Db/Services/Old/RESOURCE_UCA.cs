...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    public class RESOURCE_UCA_RepositoryTests : Init
    {
        ...
        {
            /*var model = Create(new RESOURCE_UCA
            {
        ...
        {
            /*var entity_to_create = new RESOURCE_UCA
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new RESOURCE_UCA
            {
                ID = 0, // не обновляем
            ...
            e_rereaded.Should().BeEquivalentTo(e, opt=> opt
                .Excluding(o=>o.UCA_LIST)
                .Excluding(o=>o.REQUEST)
                .Excluding(o=>o.RESOURCE)); 
            ...
            e.ID_UCA_LIST = entity_to_update.ID_UCA_LIST;
        ...
        private static RESOURCE_UCA Upd(RESOURCE_UCA u)
        {
            /*var model = new RESOURCE_UCA
            {
                ID = u.ID,
        ...
        private static RESOURCE_UCA Create(RESOURCE_UCA model)
        {
            // предпроверка
        ...
        private static RESOURCE_UCA Read(int? id)
        {
            // подготовка
            var repository = Setup();

            RESOURCE_UCA e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(RESOURCE_UCA model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(RESOURCE_UCA model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
