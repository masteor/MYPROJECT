...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class RIGHT_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        private static RIGHT_Repository Setup()
        {
            return new RIGHT_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new Права
            {
                NET_NAME = "Test_Create_NET_NAME",
                SERVER_TYPE = "Test_Create_SERVER_TYPE",
            });*/
            var model = Create(new RIGHT
            {
                ID_PROFILE = 0,
                ID_OBJECT = 0,
                ID_RIGHT_DESCR = 0,
        ...
        {
            /*var entity_to_create = new Права
            {
                NET_NAME = "TEST_CRU",
                SERVER_TYPE = "TEST_CRU",
            };*/
            var entity_to_create = new RIGHT
            {
                ID_PROFILE = 2,
                ID_OBJECT = 0,
                ID_RIGHT_DESCR = 0,
            ...
            /*var entity_to_update = new Права
            {
                ID = 0, // не обновляем
                NET_NAME = "TEST_CRU_UPDATED",
            };*/
            var entity_to_update = new RIGHT
            {
                ID = 0,
                ID_PROFILE = 3,
                ID_OBJECT = 0,
            ...
            e.ID_OBJECT = entity_to_update.ID_OBJECT;
            ...
            Read(e.ID).Should().BeEquivalentTo(entity_to_update, opt => opt
                .Excluding(o=>o.ID)
                .Excluding(o=>o.PROFILE)); // ID не проверяется, все остальные проверяются
        ...
        private static RIGHT Upd(RIGHT u)
        {
            /*var model = new Права
            {
                ID = u.ID,
        ...
        private static RIGHT Create(RIGHT model)
        {
            // предпроверка
        ...
        private static RIGHT Read(int? id)
        {
            // подготовка
            var repository = Setup();
        ...
        private void Delete(RIGHT model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(RIGHT model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
