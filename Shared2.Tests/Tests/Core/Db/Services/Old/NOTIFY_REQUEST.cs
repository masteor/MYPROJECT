...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    public class NOTIFY_REQUEST_RepositoryTests : Init
    {
        ...
        {
            /*var model = Create(new NOTIFY_REQUEST
            {
        ...
        {
            /*var entity_to_create = new NOTIFY_REQUEST
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new NOTIFY_REQUEST
            {
                ID = 0, // не обновляем
        ...
        private static NOTIFY_REQUEST Upd(NOTIFY_REQUEST u)
        {
            /*var model = new NOTIFY_REQUEST
            {
                ID = u.ID,
        ...
        private static NOTIFY_REQUEST Create(NOTIFY_REQUEST model)
        {
            // предпроверка
        ...
        private static NOTIFY_REQUEST Read(int? id)
        {
            // подготовка
            var repository = Setup();

            NOTIFY_REQUEST e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(NOTIFY_REQUEST model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(NOTIFY_REQUEST model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
