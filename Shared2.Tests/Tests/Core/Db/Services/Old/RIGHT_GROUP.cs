...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class RIGHT_GROUP_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        private static RIGHT_GROUP_Repository Setup()
        {
            return new RIGHT_GROUP_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new ГРУППЫ_ПРАВ
            {
                NET_NAME = "Test_Create_NET_NAME",
                SERVER_TYPE = "Test_Create_SERVER_TYPE",
            });*/
            var model = Create(new RIGHT_GROUP
            {
                ID_GROUP = null,
                ID_RIGHT = null,
        ...
        {
            /*var model = new ГРУППЫ_ПРАВ
            {
                ID = 0,
                NET_NAME = "Test_Delete_NET_NAME",
            };*/
            var model = new RIGHT_GROUP
            {
                ID = 0,
                ID_GROUP = null,
                ID_RIGHT = null,
        ...
        {
            /*var entity_to_create = new ГРУППЫ_ПРАВ
            {
                NET_NAME = "TEST_CRU",
                SERVER_TYPE = "TEST_CRU",
            };*/
            var entity_to_create = new RIGHT_GROUP
            {
                ID_GROUP = null,
                ID_RIGHT = null,
            ...
            /*var entity_to_update = new ГРУППЫ_ПРАВ
            {
                ID = 0, // не обновляем
                NET_NAME = "TEST_CRU_UPDATED",
            };*/
            var entity_to_update = new RIGHT_GROUP
            {
                ID = 0,
                ID_GROUP = null,
                ID_RIGHT = null,
            ...
            e.ID_GROUP = entity_to_update.ID_GROUP;
            e.ID_RIGHT = entity_to_update.ID_RIGHT;
            ...
            Read(e.ID).Should()
                .BeEquivalentTo(entity_to_update,
                    opt => opt.Excluding(o => o.ID)); // ID не проверяется, все остальные проверяются
        ...
        private static RIGHT_GROUP Upd(RIGHT_GROUP u)
        {
            /*var model = new ГРУППЫ_ПРАВ
            {
                ID = u.ID,
        ...
        private static RIGHT_GROUP Create(RIGHT_GROUP model)
        {
            // предпроверка
            ...
            Action
                act_add = () =>
                    repository.Add(
                        model); // acF меняется по ссылке, это значит в вызывающей процедуре он тоже поменяется
        ...
        private static RIGHT_GROUP Read(int? id)
        {
            // подготовка
            var repository = Setup();

            RIGHT_GROUP e_readed;
            // действие
            if (id != null)
            {
        ...
        private void Delete(RIGHT_GROUP model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(RIGHT_GROUP model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
