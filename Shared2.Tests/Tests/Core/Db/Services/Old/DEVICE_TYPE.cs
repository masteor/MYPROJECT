...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    public class DEVICE_TYPE_RepositoryTests : Init
    {
        ...
        {
            /*var model = Create(new DEVICE_TYPE
            {
        ...
        {
            /*var entity_to_create = new DEVICE_TYPE
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new DEVICE_TYPE
            {
                ID = 0, // не обновляем
        ...
        private static DEVICE_TYPE Upd(DEVICE_TYPE u)
        {
            /*var model = new DEVICE_TYPE
            {
                ID = u.ID,
        ...
        private static DEVICE_TYPE Create(DEVICE_TYPE model)
        {
            // предпроверка
        ...
        private static DEVICE_TYPE Read(int? id)
        {
            // подготовка
            var repository = Setup();

            DEVICE_TYPE e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(DEVICE_TYPE model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(DEVICE_TYPE model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
