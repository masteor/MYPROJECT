...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class SECRET_TYPE_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        private static SECRET_TYPE_Repository Setup()
        {
            return new SECRET_TYPE_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new SECRET_TYPE
            {
        ...
        {
            /*var entity_to_create = new SECRET_TYPE
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new SECRET_TYPE
            {
                ID = 0, // не обновляем
        ...
        private static SECRET_TYPE Upd(SECRET_TYPE u)
        {
            /*var model = new SECRET_TYPE
            {
                ID = u.ID,
        ...
        private static SECRET_TYPE Create(SECRET_TYPE model)
        {
            // предпроверка
        ...
        private static SECRET_TYPE Read(int? id)
        {
            // подготовка
            var repository = Setup();

            SECRET_TYPE e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(SECRET_TYPE model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(SECRET_TYPE model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
