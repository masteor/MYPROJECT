...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class ARM_USER_ROLE_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        public static ARM_USER_ROLE_Repository Setup()
        {
            return new ARM_USER_ROLE_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new ARM_USER_ROLE
            {
        ...
        {
            /*var entity_to_create = new ARM_USER_ROLE
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new ARM_USER_ROLE
            {
                ID = 0, // не обновляем
            ...
            Read(e.ID).Should().BeEquivalentTo(entity_to_update, opt => opt
                .Excluding(o=>o.ID)
                .Excluding(o=>o.ARM_USER)); // ID не проверяется, все остальные проверяются
        ...
        private static ARM_USER_ROLE Upd(ARM_USER_ROLE u)
        {
            /*var model = new ARM_USER_ROLE
            {
                ID = u.ID,
        ...
        private static ARM_USER_ROLE Create(ARM_USER_ROLE model)
        {
            // предпроверка
        ...
        private static ARM_USER_ROLE Read(int? id)
        {
            // подготовка
            var repository = Setup();

            ARM_USER_ROLE e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(ARM_USER_ROLE model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(ARM_USER_ROLE model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
