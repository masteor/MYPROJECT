...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class NOTIFY_SUB_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        private static NOTIFY_SUB_Repository Setup()
        {
            return new NOTIFY_SUB_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new NOTIFY_SUB
            {
        ...
        {
            /*var entity_to_create = new NOTIFY_SUB
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new NOTIFY_SUB
            {
                ID = 0, // не обновляем
            ...
            e.PRODUCER = entity_to_update.PRODUCER;
            e.SUBSCRIBER = entity_to_update.SUBSCRIBER;
            ...
            Read(e.ID).Should().BeEquivalentTo(entity_to_update, opt => opt
                .Excluding(o=>o.ID)
                .Excluding(o=>o.STAFF_UNIT)
                .Excluding(o=>o.STAFF_UNIT1)); // ID не проверяется, все остальные проверяются
        ...
        private static NOTIFY_SUB Upd(NOTIFY_SUB u)
        {
            /*var model = new NOTIFY_SUB
            {
                ID = u.ID,
        ...
        private static NOTIFY_SUB Create(NOTIFY_SUB model)
        {
            // предпроверка
        ...
        private static NOTIFY_SUB Read(int? id)
        {
            // подготовка
            var repository = Setup();
        ...
        private void Delete(NOTIFY_SUB model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(NOTIFY_SUB model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
