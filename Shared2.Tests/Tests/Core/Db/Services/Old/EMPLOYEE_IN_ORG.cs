...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class EMPLOYEE_IN_ORG_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        public static EMPLOYEE_IN_ORG_Repository Setup()
        {
            return new EMPLOYEE_IN_ORG_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new EMPLOYEE_IN_ORG
            {
        ...
        public void TEST_GetAll__DoesNotThrow()
        {
            ...
            var sd = repository.GetAll();
            var y37h = repository.GetById(350);
        ...
        {
            /*var entity_to_create = new EMPLOYEE_IN_ORG
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new EMPLOYEE_IN_ORG
            {
                ID = 0, // не обновляем
            ...
            e.ID_REQUEST_1 = e.ID_REQUEST_1;
            e.ID_REQUEST_2 = e.ID_REQUEST_2;
        ...
        private static EMPLOYEE_IN_ORG Upd(EMPLOYEE_IN_ORG u)
        {
            /*var model = new EMPLOYEE_IN_ORG
            {
                ID = u.ID,
        ...
        private static EMPLOYEE_IN_ORG Create(EMPLOYEE_IN_ORG model)
        {
            // предпроверка
        ...
        private static EMPLOYEE_IN_ORG Read(int? id)
        {
            // подготовка
            var repository = Setup();

            EMPLOYEE_IN_ORG e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(EMPLOYEE_IN_ORG model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(EMPLOYEE_IN_ORG model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
