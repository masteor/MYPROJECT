...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class ARM_USER_RepositoryTests : Init
    {
    /// <summary>
    /// настройка 
    /// </summary>
    public static ARM_USER_Repository Setup()
    {
        return new ARM_USER_Repository(DB_FACTORY);
    }

    [Test]
    public void TEST_Create()
    {
        /*var model = Create(new ARM_USER
        {
        ...
        var model = Create(new ARM_USER
        {
            ...
            идАРМа = 1,
            идРолиСотрудника = 1,
            идЗаявкиРазрешенияДопуска = 1,
            идЗаявкиПрекращенияДоступа = null,
            
            ARM = null,
            ЗаявкаРазрешенияДопуска = null,
            ЗаявкаПрекращенияДопуска = null,
            Сотрудник = null,
            ARM_USER_ROLE = null
        });
    ...
    {
        /*var entity_to_create = new ARM_USER
        {
            NET_NAME = "TEST_CRU",
        ...
        /*var entity_to_update = new ARM_USER
        {
            ID = 0, // не обновляем
        ...
        e.идАРМа = entity_to_update.идАРМа;
        e.идРолиСотрудника = entity_to_update.идРолиСотрудника;
        e.идСотрудника = entity_to_update.идСотрудника;
        e.идЗаявкиРазрешенияДопуска = entity_to_update.идЗаявкиРазрешенияДопуска;
        e.идЗаявкиПрекращенияДоступа = entity_to_update.идЗаявкиПрекращенияДоступа;
        ...
        Read(e.ID).Should().BeEquivalentTo(entity_to_update, opt => opt
            .Excluding(o => o.ID)
            .Excluding(o=>o.ARM)
            .Excluding(o=>o.ARM_USER_ROLE)
            .Excluding(o=>o.ЗаявкаРазрешенияДопуска)
            .Excluding(o=>o.ЗаявкаПрекращенияДопуска)
            .Excluding(o=>o.Сотрудник)); // ID не проверяется, все остальные проверяются
        ...
        Read(e.ID).Should()
            .NotBeEquivalentTo(entity_to_update, opt => opt.Including(o => o.ID)); // проверяется только ID
    ...
    private static ARM_USER Upd(ARM_USER u)
    {
        /*var model = new ARM_USER
        {
            ID = u.ID,
        ...
        {
            ID = u.ID,
            идСотрудника = u.идСотрудника,
            идАРМа = u.идАРМа,
            идРолиСотрудника = u.идРолиСотрудника,
            идЗаявкиРазрешенияДопуска = u.идЗаявкиРазрешенияДопуска,
            идЗаявкиПрекращенияДоступа = u.идЗаявкиПрекращенияДоступа,
            ARM = u.ARM,
            ЗаявкаРазрешенияДопуска = u.ЗаявкаРазрешенияДопуска,
            ЗаявкаПрекращенияДопуска = u.ЗаявкаПрекращенияДопуска,
            Сотрудник = u.Сотрудник,
            ARM_USER_ROLE = u.ARM_USER_ROLE
        };
        return model;
    }

    private static ARM_USER Create(ARM_USER model)
    {
        // предпроверка
    ...
    private static ARM_USER Read(int? id)
    {
        // подготовка
        var repository = Setup();
    ...
    private void Delete(ARM_USER model)
    {
        // подготовка
        var repository = Setup();
    ...
    private static void Update(ARM_USER model)
    {
        // подготовка
        var repository = Setup();
        Assert.NotNull(model);
