...
namespace DBPSA.Shared.Tests.Core.Db.Services
{
    ...
    /// </summary>
    [TestFixture]
    public class RESOURCE_RESP_RepositoryTests : Init
    {
        /// <summary>
        /// настройка 
        /// </summary>
        private static RESOURCE_RESP_Repository Setup()
        {
            return new RESOURCE_RESP_Repository(DB_FACTORY);
        }

        [Test]
        public void TEST_Create()
        {
            /*var model = Create(new RESOURCE_RESP
            {
        ...
        {
            /*var entity_to_create = new RESOURCE_RESP
            {
                NET_NAME = "TEST_CRU",
            ...
            /*var entity_to_update = new RESOURCE_RESP
            {
                ID = 0, // не обновляем
            ...
            e.ID_RESOURCE = entity_to_update.ID_RESOURCE;
        ...
        private static RESOURCE_RESP Upd(RESOURCE_RESP u)
        {
            /*var model = new RESOURCE_RESP
            {
                ID = u.ID,
        ...
        private static RESOURCE_RESP Create(RESOURCE_RESP model)
        {
            // предпроверка
        ...
        private static RESOURCE_RESP Read(int? id)
        {
            // подготовка
            var repository = Setup();

            RESOURCE_RESP e_readed;
            // действие
            if (id != null) 
            {
        ...
        private void Delete(RESOURCE_RESP model)
        {
            // подготовка
            var repository = Setup();
        ...
        private static void Update(RESOURCE_RESP model)
        {
            // подготовка
            var repository = Setup();
            Assert.NotNull(model);
