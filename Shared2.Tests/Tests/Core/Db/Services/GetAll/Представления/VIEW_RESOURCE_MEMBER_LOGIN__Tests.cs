...
{
    [TestFixture]
    public class VIEW_RESOURCE_MEMBER_LOGIN__Tests : Init
    {
        private const string ИМЯ_ТАБЛИЦЫ = ИмяТаблицы.VIEW_RESOURCE_MEMBER_EMPLOYEE;
        ...
        public void TEST_SqlQuery__DoesNotThrow()
        {
            // подготовка
            var repository = Setup();
            List<VIEW_RESOURCE_MEMBER_EMPLOYEE>? all = null;
            
            // действие
            void TestDelegate() 
                => all = repository
                    .GetContext()
                    .ViewResourceMemberLogins
        ...
        private static VIEW_RESOURCE_MEMBER_EMPLOYEE_Repository Setup() => new VIEW_RESOURCE_MEMBER_EMPLOYEE_Repository(DB_FACTORY);
    }
}