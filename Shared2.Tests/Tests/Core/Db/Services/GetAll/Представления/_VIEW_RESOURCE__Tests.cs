...
{
    [TestFixture]
    public class VIEW_RESOURCE__Tests : Init
    {
        private const string ИМЯ_ТАБЛИЦЫ = ИмяТаблицы.ViewResource;
        ...
        public void TEST_SqlQuery__DoesNotThrow()
        {
            ...
            void TestDelegate() 
                => all = repository
                    .GetContext()
                    .ViewResources
                    .SqlQuery($"select * from {ИмяТаблицы.ViewResource} ")
                    .ToList();
...
//            Assert.IsNotEmpty(all);
        ...
        public void Проверка_целостности_полей_таблицы_в_бд()
        {
            ...
            IEnumerable<string> поляВоПрограмме = new string[]{
                ...
                ИмяКолонки.OWNER,
                ИмяКолонки.RESPONSIBLE,
                ИмяКолонки.SERVICE_TYPE_NAME,
                ИмяКолонки.AC_FRAGMENT,
        ...
        private static VIEW_RESOURCE_Repository Setup() => new VIEW_RESOURCE_Repository(DB_FACTORY);
    }
}