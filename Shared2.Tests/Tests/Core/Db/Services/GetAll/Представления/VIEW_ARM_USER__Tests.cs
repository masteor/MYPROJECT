...
{
    [TestFixture]
    public class VIEW_ARM_USER__Tests : Init
    {
        [Test]
        ...
        public void TEST_SqlQuery__DoesNotThrow()
        {
            ...
            void TestDelegate() 
                => all = repository
                    .GetContext()
                    .ViewArmUsers
                    .SqlQuery($"select * from {ИмяТаблицы.ViewArmUser} ")
                    .ToList();
        ...
        public void Проверка_наличия_таблицы_в_бд()
        {
            ...
            (_sysrlRepository.Найти(r => 
                    r.S13.Trim(' ') == ИмяТаблицы.ViewArmUser)
                .S13.Trim(' ') == ИмяТаблицы.ViewArmUser);
        }
        ...
        public void Проверка_целостности_полей_таблицы_в_бд()
        {
            var idTable = _sysrlRepository.Найти(r => 
                r.S13.Trim(' ') == ИмяТаблицы.ViewArmUser).S11;
            ...
            IEnumerable<string> поляВоПрограмме = new string[]{
                ...
                ИмяКолонки.SFIO,
                ИмяКолонки.TNUM,
                ИмяКолонки.ARM_REG_NUM,
                ИмяКолонки.ARM_USER_ROLE_NAME,
        ...
        private static VIEW_ARM_USER_Repository Setup() => new VIEW_ARM_USER_Repository(DB_FACTORY);
    }
}