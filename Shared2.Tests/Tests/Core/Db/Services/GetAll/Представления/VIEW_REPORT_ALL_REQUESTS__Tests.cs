...
{
    [TestFixture]
    public class VIEW_REPORT_ALL_REQUESTS__Tests : Init
    {
        private const string ИМЯ_ТАБЛИЦЫ = ИмяТаблицы.ViewReportAllRequests;
        ...
        public void TEST_SqlQuery__DoesNotThrow()
        {
            // подготовка
            var repository = Setup();
            List<VIEW_REPORT_ALL_REQUESTS>? all = null;
            
            // действие
            void TestDelegate() 
                => all = repository
                    .GetContext()
                    .ViewReportAllRequests
                    .SqlQuery($"select * from {ИмяТаблицы.ViewReportAllRequests} ")
                    .ToList();
        ...
        public void Проверка_целостности_полей_таблицы_в_бд()
        {
            ...
            IEnumerable<string> поляВоПрограмме = new string[]{
                ...
                ИмяКолонки.REQUEST_TYPE_NAME,
                ИмяКолонки.REQUEST_TYPE_CODE,
                ИмяКолонки.REQUEST_STATE_NAME,
                ИмяКолонки.ID_DOC,
        ...
        private static VIEW_REPORT_ALL_REQUESTS_Repository Setup() => new VIEW_REPORT_ALL_REQUESTS_Repository(DB_FACTORY);
    }
}