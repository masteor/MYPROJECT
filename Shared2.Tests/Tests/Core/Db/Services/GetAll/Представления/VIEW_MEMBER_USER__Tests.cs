...
{
    [TestFixture]
    public class VIEW_MEMBER_USER__Tests : Init
    {
        private const string ИМЯ_ТАБЛИЦЫ = ИмяТаблицы.VIEW_MEMBER_USER;
        ...
        public void TEST_SqlQuery__DoesNotThrow()
        {
            // подготовка
            var repository = Setup();
            List<VIEW_MEMBER_USER>? all = null;
...
//            Assert.IsNotEmpty(all);
        ...
        private static VIEW_MEMBER_USER_Repository Setup() => new VIEW_MEMBER_USER_Repository(DB_FACTORY);
    }
}